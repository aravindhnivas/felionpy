...
def main(gauss_args):
    ...
    for method in normMethods:
        ...
        for i, guess_amp in enumerate(_guess_inten):
            fit_guesses[f"A{i}"] = guess_amp
        ...
        if felix:
            with open(datfile, "r") as f:
                writeFileName_datfile = f.readlines()
        if gauss_args["writeFile"]:

            print("Writing file")
            with open(fitfile, ("a+", "w+")[overwrite]) as f:
                if overwrite:
                    f.write(f"#Wn(cm-1)\tIntensity\n")
                for freq, inten in zip(_wn, sim_inten):
                    f.write(f"{freq:.4f}\t{inten:.4f}\n")

            with open(expfile, ("a+", "w+")[overwrite]) as f:

                if overwrite:
                    f.write(
                        f"#Frequency\tFreq_err\t#Intensity({method})\t#Intensity_err\t#Sigma\t#Sigma_err\t#FWHM\t#FWHM_err\n"
                    )
                ...
                for freq, amp, sigma in zip(*fitted):
                    ...
                    f.write(
                        f"{freq[0]:.4f}\t{freq[1]:.4f}\t{amp[0]:.4f}\t{amp[1]:.4f}\t{sigma[0]:.4f}\t{sigma[1]:.4f}\t{_fwhm[0]:.4f}\t{_fwhm[1]:.4f}\n"
                    )
            writeFileName = pt(gauss_args["writeFileName"]).stem
            ...
            if writeFileName != "":
                ...
                for read, write in zip(
                    [expfile, fitfile, datfile], [writeFileName_expfile, writeFileName_fitfile, writeFileName_datfile]
                ):
